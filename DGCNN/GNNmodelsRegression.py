...
class simple_DGCNN_plus_regression(nn.Module):
    def __init__(self, hidden_channels, output_dim=1, dropout_prob=0.5, input_channel=1,
                 estimate_uncertainty=False, activation='identity', k=5):
        super(simple_DGCNN_plus_regression, self).__init__()
        # DynamicEdgeConv layers (EdgeConv with dynamic KNN graph)
        # Each layer uses an MLP to compute messages from a neighbor pair (i,j)
        self.conv1 = DynamicEdgeConv(
            nn=Sequential(
                Linear(2 * input_channel, hidden_channels),  # MLP input: concat of two node feats
                ReLU(),
        ...
        # Batch normalization for each layer's output
        self.bn1 = BatchNorm1d(hidden_channels)
        self.bn2 = BatchNorm1d(hidden_channels)
        self.bn3 = BatchNorm1d(hidden_channels)
        self.bn4 = BatchNorm1d(hidden_channels)
        ...
        # Regression head (as defined earlier) for final prediction
        self.regression_head = RegressionHead(
            hidden_dim=hidden_channels, output_dim=output_dim,
            dropout_prob=dropout_prob, estimate_uncertainty=estimate_uncertainty,
            activation=activation
        )
    
    def forward(self, x, edge_index, batch):
        # x: node feature matrix (shape [num_nodes, input_channel]); 
        # edge_index: not used (dynamic graph is computed internally);
        # batch: batch index for each node (shape [num_nodes]) indicating graph ID.
        # Layer 1: dynamic conv on initial features
        x1 = F.relu(self.conv1(x, batch=batch))         # Compute 1-hop neighbors in feature space and aggregate
        x1 = self.bn1(x1)                               # Batch norm
        # Layer 2 with residual connection
        x2 = F.relu(self.conv2(x1, batch=batch))        # Dynamic conv on features x1
        x2 = self.dropout(x2)
        x2 = self.bn2(x2)
        x2 = x2 + x1                                    # Residual connection: add output of prev layer
        # Layer 3 with residual
        x3 = F.relu(self.conv3(x2, batch=batch))
        x3 = self.dropout(x3)
        x3 = self.bn3(x3)
        x3 = x3 + x2
        # Layer 4 with residual
        x4 = F.relu(self.conv4(x3, batch=batch))
        x4 = self.dropout(x4)
        x4 = self.bn4(x4)
        x4 = x4 + x3
        # Layer 5 with residual
        x5 = F.relu(self.conv5(x4, batch=batch))
        x5 = self.dropout(x5)
        x5 = self.bn5(x5)
        x5 = x5 + x4
        # Global pooling to get graph-level embedding
        graph_emb = global_mean_pool(x5, batch)        # shape [batch_size, hidden_channels]
        # Save the graph embedding for downstream use (e.g., feature extraction)
        feat = graph_emb
        # Regression head to predict target value
        out = self.regression_head(graph_emb)          # shape [batch_size, output_dim]
        return out, feat
