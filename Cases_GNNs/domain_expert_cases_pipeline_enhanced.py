...
class DomainExpertCasesPipeline(MixedEmbeddingPipeline):
    ...
    def _display_metrics_with_std(self, model_type, metrics, fold_metrics):
        """Helper function to display metrics with standard deviation"""
        mse_values = [fold['mse'] for fold in fold_metrics]
        r2_values = [fold['r2'] for fold in fold_metrics]
        
        mse_std = np.std(mse_values)
        r2_std = np.std(r2_values)
        
        print(f"{model_type.upper()}:")
        print(f"  MSE: {metrics['mse']:.4f} ± {mse_std:.4f}")
        print(f"  R²:  {metrics['r2']:.4f} ± {r2_std:.4f}")
    ...
    def _save_case_results(self, results, target_name):
        """Save case-specific results with enhanced metrics reporting"""
        with open(f"{self.save_dir}/{self.case_type}_{target_name}_results.pkl", 'wb') as f:
            pickle.dump(results, f)
        
        # Calculate and display mean ± std for each model
        print(f"\n{'='*60}")
        ...
        for phase in ['knn', 'explainer']:
            if phase in results:
                ...
                for model_type, result in results[phase].items():
                    ...
                    fold_metrics = result['fold_metrics']  # This contains individual fold results
                    
                    # Calculate std from fold results
                    mse_values = [fold['mse'] for fold in fold_metrics]
                    r2_values = [fold['r2'] for fold in fold_metrics]
        ...
        if 'ml_models' in results:
            print(f"ML MODELS ON EMBEDDINGS:")
            print("-" * 40)
    ...
    def _save_case4_combined_results(self, combined_results, ace_target_idx):
        """Save combined Case 4 results with enhanced metrics reporting"""
        # Save combined results
        with open(f"{self.save_dir}/case4_combined_results.pkl", 'wb') as f:
            pickle.dump(combined_results, f)
        
        # Display combined results summary with mean ± std
        print(f"\n{'='*60}")
        print(f"CASE 4 COMBINED RESULTS SUMMARY")
        print(f"{'='*60}")
    ...
    def _save_case5_combined_results(self, combined_results, h2_target_idx):
        """Save combined Case 5 results with enhanced metrics reporting"""
        # Save combined results
        ...
        # Display combined results summary with mean ± std
        print(f"\n{'='*60}")
        print(f"CASE 5 COMBINED RESULTS SUMMARY")
        print(f"{'='*60}")
