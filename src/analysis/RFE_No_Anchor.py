...
def create_directories():
    ...
    directories = [
        'results_rfe_nested_cv/plots',
        'results_rfe_nested_cv/metrics',
        'results_rfe_nested_cv/selected_features'
    ]
...
def create_performance_plot(all_actual, all_predictions, target, n_features, final_r2, final_mse, model_type):
    ...
    # Create scatter plot
    sns.scatterplot(x=all_actual, y=all_predictions, alpha=0.6, color='#666666')
    ...
    title = f'{target} Prediction ({model_type})\nTop {feature_display} Features' if n_features else f'{target} Prediction ({model_type})\nAll Features'
    plt.title(f'{title}\n{metrics}')
    ...
    filename = f'results_rfe_nested_cv/plots/results_{target}_{feature_suffix}_features_{model_type}.png'
...
def select_features_with_rfe(X, y, n_features=None, model_type='extratrees'):
    ...
    print(f"    RFE completed in {time.time() - start_time:.2f}s")
...
def _train_and_evaluate_once(X, y, train_idx, val_idx, n_features, model_type):
    ...
    if n_features is None:
        selected_features = X_train.columns.tolist()  # Use all features
...
def _inner_loop_select(X, y, model_type):
    ...
    inner_kf = KFold(n_splits=3, shuffle=True, random_state=42)  # Reduced from 5 to 3
    best_score = -float('inf')
    best_n_features = None
    all_combinations_results = []
    
    # Define hyperparameter search space - matching reference code
    n_features_options = [None, 50, 75, 100]  # None means use all features
    
    print(f"    Inner CV: Testing {len(n_features_options)} feature count combinations...")
    print(f"    {'Combination':<15} {'N_Features':<12} {'Mean R²':<10} {'Std R²':<10} {'Best':<6}")
    print(f"    {'-'*60}")
    ...
    for i, n_features in enumerate(n_features_options):
        ...
        for tr_idx, val_idx in inner_kf.split(X):
            r2_score = _train_and_evaluate_once(X, y, tr_idx, val_idx, n_features, model_type)
        ...
        print(f"    {i+1:<15} {feature_display:<12} {mean_val:<10.4f} {std_val:<10.4f} {best_marker:<6}")
    
    print(f"    {'-'*60}")
    feature_display = "All" if best_n_features is None else str(best_n_features)
    print(f"    Best hyperparameters: n_features={feature_display} (R² = {best_score:.4f})")
    ...
    for i, result in enumerate(sorted_results[:3]):
        ...
        print(f"    {rank}. n_features={feature_display}")
...
def run_model_nested_cv(data_path, target="ACE-km", model_type='extratrees'):
    """
    Run model with nested CV hyperparameter tuning for feature selection
    ...
    # Define target columns to exclude from features (matching reference code)
    ...
    # Convert all columns to numeric (matching reference code)
    ...
    # Nested CV setup - reduced outer folds
    outer_kf = KFold(n_splits=3, shuffle=True, random_state=42)  # Reduced from 5 to 3
    outer_results = []
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_kf.split(X)):
        fold_num = fold + 1
        print(f"\n  {'-'*50}")
        print(f"  OUTER FOLD {fold_num}/3")  # Updated to reflect 3 folds
        print(f"  {'-'*50}")
        ...
        if r2 > best_outer_r2:
            best_outer_r2 = r2
            best_n_features = best_n_features_inner
            # For tree-based models, we can save the model
    ...
    print(f"Target: {target}")
    print(f"Number of outer folds: 3")  # Updated
    ...
    for result in outer_results:
        ...
        print(f"{fold:<6} {r2:<10.4f} {mse:<10.4f} {best_n_features:<15}")
    ...
    for n_features, count in n_features_counts.items():
        feature_display = "All" if n_features is None else str(n_features)
        print(f"    {feature_display}: {count}/3 folds")  # Updated
    
    # Collect all predictions and targets for final plot
    print(f"\nCollecting predictions for final plot...")
    all_predictions = []
    all_actual = []
    
    for result in outer_results:
        # Recreate the model for this fold to get predictions
        fold_train_idx = list(range(result['train_size']))
        fold_test_idx = list(range(result['train_size'], result['train_size'] + result['test_size']))
        
        X_fold_train, X_fold_test = X.iloc[fold_train_idx], X.iloc[fold_test_idx]
        y_fold_train, y_fold_test = y.iloc[fold_train_idx], y.iloc[fold_test_idx]
        
        # Use the best n_features for this fold
        selected_features = result['selected_features']
        X_fold_train_selected = X_fold_train[selected_features]
        X_fold_test_selected = X_fold_test[selected_features]
        
        # Standard scaling
        if model_type == 'linearsvr':
            scaler = StandardScaler()
            X_fold_train_scaled = scaler.fit_transform(X_fold_train_selected)
            X_fold_test_scaled = scaler.transform(X_fold_test_selected)
            model = LinearSVR(random_state=42, max_iter=100000, tol=1e-4, dual=True)
        else:
            X_fold_train_scaled, X_fold_test_scaled = X_fold_train_selected, X_fold_test_selected
            model = ExtraTreesRegressor(n_estimators=200, random_state=42)
        ...
        all_actual.extend(y_fold_test)
    
    # Create final plot
    create_performance_plot(all_actual, all_predictions, target, best_n_features, avg_r2, avg_mse, model_type)
    ...
    results_path = f'results_rfe_nested_cv/metrics/results_{target}_{model_type}_nested_cv.csv'
    ...
    features_path = f'results_rfe_nested_cv/selected_features/{target}_{model_type}_feature_frequency.csv'
...
def save_hyperparameter_summary(outer_results, target, model_type):
    ...
    csv_path = f'results_rfe_nested_cv/metrics/{target}_{model_type}_hyperparameter_summary.csv'
...
if __name__ == "__main__":
    ...
    for target in main_targets:
        for model_type in model_types:
            print(f"\n{'='*80}")
            print(f"Running NESTED CV for {target} with {model_type.upper()}")
            print(f"{'='*80}")
    ...
    if all_results:
        ...
        overall_path = 'results_rfe_nested_cv/metrics/overall_results_nested_cv.csv'
