...
class DomainExpertCasesPipeline(MixedEmbeddingPipeline):
    ...
    def _run_case1(self):
        """Case 1: Use only hydrogenotrophic features for both H2-km and ACE-km targets"""
        print("Case 1: Using only hydrogenotrophic features for both H2-km and ACE-km targets")
        print("Targets: H2-km and ACE-km")
    ...
    def _run_case2(self):
        """Case 2: Use only acetoclastic features for both H2-km and ACE-km targets"""
        print("Case 2: Using only acetoclastic features for both H2-km and ACE-km targets")
        print("Targets: H2-km and ACE-km")
        print(f"Anchored features: {len(self.anchored_features)} features")
    ...
    def _run_case3(self):
        """Case 3: Use all feature groups for both H2-km and ACE-km targets"""
        print("Case 3: Using acetoclastic + hydrogenotrophic + syntrophic features for both H2-km and ACE-km targets")
        print("Targets: H2-km and ACE-km")
        print(f"Anchored features: {len(self.anchored_features)} features")
    ...
    def _create_case3_combined_visualization(self, combined_results, ace_target_idx, h2_target_idx):
        """Create combined visualization for Case 3 showing both targets"""
        print(f"\nCreating combined Case 3 visualization for both targets...")
        
        # Create figure with subplots for both targets
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle('Case 3: Combined Results for Both Targets\nAll Feature Groups (Acetoclastic + Hydrogenotrophic + Syntrophic)', fontsize=16, fontweight='bold')
        ...
        colors = ['#FF6B6B', '#4ECDC4']  # Red for ACE-km, Teal for H2-km
        labels = ['ACE-km', 'H2-km']
        
        for phase_idx, (phase, phase_name) in enumerate(plot_configs):
            # ACE-km plots (top row)
            ax_ace = axes[0, phase_idx]
            ace_results = combined_results['ace_km']
            
            # Find best model for ACE-km
            best_ace_model = None
            best_ace_r2 = -float('inf')
            if phase == 'ml':
                for model_type, result in ace_results.get('ml_models', {}).items():
                    if result['avg_metrics']['r2'] > best_ace_r2:
                        best_ace_r2 = result['avg_metrics']['r2']
                        best_ace_model = model_type
            else:
                for model_type, result in ace_results.get(phase, {}).items():
                    if result['avg_metrics']['r2'] > best_ace_r2:
                        best_ace_r2 = result['avg_metrics']['r2']
                        best_ace_model = model_type
            
            # Plot ACE-km results
            if best_ace_model and 'fold_predictions' in ace_results.get(phase if phase != 'ml' else 'ml_models', {}).get(best_ace_model, {}):
                result_data = ace_results.get(phase if phase != 'ml' else 'ml_models', {}).get(best_ace_model, {})
                actual = []
                pred = []
                for fold_data in result_data['fold_predictions']:
                    actual.extend(fold_data['actual'])
                    pred.extend(fold_data['predicted'])
                
                ax_ace.scatter(actual, pred, c=colors[0], alpha=0.7, s=60, edgecolors='black', linewidth=0.5)
                
                # Add perfect prediction line
                min_val = min(min(actual), min(pred))
                max_val = max(max(actual), max(pred))
                ax_ace.plot([min_val, max_val], [min_val, max_val], 'k--', alpha=0.8, linewidth=2)
                
                # Formatting
                ax_ace.set_xlabel('Actual ACE-km Values', fontsize=12)
                ax_ace.set_ylabel('Predicted ACE-km Values', fontsize=12)
                ax_ace.set_title(f'{phase_name} - ACE-km\nR² = {best_ace_r2:.4f}', fontsize=11, fontweight='bold')
                ax_ace.grid(True, alpha=0.3)
            
            # H2-km plots (bottom row)
            ax_h2 = axes[1, phase_idx]
            h2_results = combined_results['h2_km']
            
            # Find best model for H2-km
            best_h2_model = None
            best_h2_r2 = -float('inf')
            if phase == 'ml':
                for model_type, result in h2_results.get('ml_models', {}).items():
                    if result['avg_metrics']['r2'] > best_h2_r2:
                        best_h2_r2 = result['avg_metrics']['r2']
                        best_h2_model = model_type
            else:
                for model_type, result in h2_results.get(phase, {}).items():
                    if result['avg_metrics']['r2'] > best_h2_r2:
                        best_h2_r2 = result['avg_metrics']['r2']
                        best_h2_model = model_type
            
            # Plot H2-km results
            if best_h2_model and 'fold_predictions' in h2_results.get(phase if phase != 'ml' else 'ml_models', {}).get(best_h2_model, {}):
                result_data = h2_results.get(phase if phase != 'ml' else 'ml_models', {}).get(best_h2_model, {})
                actual = []
                pred = []
                for fold_data in result_data['fold_predictions']:
                    actual.extend(fold_data['actual'])
                ...
                ax_h2.scatter(actual, pred, c=colors[1], alpha=0.7, s=60, edgecolors='black', linewidth=0.5)
                
                # Add perfect prediction line
                min_val = min(min(actual), min(pred))
                max_val = max(max(actual), max(pred))
                ax_h2.plot([min_val, max_val], [min_val, max_val], 'k--', alpha=0.8, linewidth=2)
                
                # Formatting
                ax_h2.set_xlabel('Actual H2-km Values', fontsize=12)
                ax_h2.set_ylabel('Predicted H2-km Values', fontsize=12)
                ax_h2.set_title(f'{phase_name} - H2-km\nR² = {best_h2_r2:.4f}', fontsize=11, fontweight='bold')
                ax_h2.grid(True, alpha=0.3)
        
        # Add legend
        legend_elements = [plt.scatter([], [], c=colors[0], s=60, label=labels[0]),
                          plt.scatter([], [], c=colors[1], s=60, label=labels[1])]
...
def run_all_cases(data_path="Data/New_Data.csv"):
    ...
    cases = ['case1', 'case2', 'case3']  # Only run cases 1, 2, 3 with both targets
    all_results = {}
...
def test_single_case(data_path="Data/New_Data.csv", case_type='case1'):
    ...
    try:
        pipeline = DomainExpertCasesPipeline(
            ...
            num_folds=5,    # Reduced for testing
...
if __name__ == "__main__":
    ...
    print("Testing single case with nested CV...")
