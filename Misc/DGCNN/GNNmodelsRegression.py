...
class Enhanced_DGCNN_regression(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        for i, (conv, bn) in enumerate(zip(self.conv_layers, self.bn_layers)):
            x_new = F.relu(conv(x, batch=batch))  # DynamicEdgeConv needs batch parameter
            x_new = bn(x_new)
        ...
        if batch is not None:
            ...
            std_feat = torch.std(x, dim=0, keepdim=True)
            x_std = std_feat
...
class simple_DGCNN_plus_regression(nn.Module):
    """
    Simple DGCNN model for regression with embedding extraction capability.
    This is the "plus" version that returns both predictions and embeddings.
    """
    def __init__(self, hidden_channels, output_dim=1, dropout_prob=0.5, input_channel=1,
                 estimate_uncertainty=False, activation='identity', k=5, num_layers=3):
        super(simple_DGCNN_plus_regression, self).__init__()
        
        self.num_layers = num_layers
        self.k = k
        
        # Simple DynamicEdgeConv layers
        self.conv_layers = nn.ModuleList()
        self.bn_layers = nn.ModuleList()
    ...
    def forward(self, x, edge_index, batch):
        # Forward through DGCNN layers
        for i, (conv, bn) in enumerate(zip(self.conv_layers, self.bn_layers)):
            x = F.relu(conv(x, batch=batch))
            x = bn(x)
            x = self.dropout(x)
        ...
        # Store embeddings for feature extraction
        feat = x_pooled
