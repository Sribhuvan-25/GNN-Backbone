...
class MixedEmbeddingPipeline:
    ...
    def train_ml_models(self, embeddings, targets, target_idx):
        ...
        if targets.ndim > 1:
            target_values = targets[:, target_idx]  # Extract specific target column
        else:
            target_values = targets  # Already 1D
        ...
        for model_name, model in ml_models.items():
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(embeddings)):
                ...
                y_train, y_val = target_values[train_idx], target_values[val_idx]  # Use extracted target values
    ...
    def plot_results(self, gnn_results, ml_results, target_idx):
        ...
        base_colors = ['skyblue', 'lightcoral', 'lightgreen', 'orange', 'purple', 'yellow', 'pink', 'cyan', 'brown']
        colors = [base_colors[i % len(base_colors)] for i in range(len(all_models))]
        
        bars3 = ax3.bar(range(len(all_models)), all_r2_scores, color=colors)
        ...
        bars4 = ax4.bar(range(len(all_models)), all_rmse_scores, color=colors)
    ...
    def run_pipeline(self):
        ...
        for target_idx, target_name in enumerate(self.target_names):
            ...
            # Clean up any existing batch attributes from previous target processing
            self._cleanup_data_objects()
    ...
    def _cleanup_data_objects(self):
        """Clean up any batch attributes from data objects to prevent conflicts"""
        print("Cleaning up data objects for next target...")
        for data in self.dataset.data_list:
            # Remove batch attribute if it exists
    ...
    def plot_ml_model_results(self, ml_results, target_name, embeddings_source):
        ...
        # Ensure we have enough colors by cycling through them
        base_colors = ['skyblue', 'lightcoral', 'lightgreen', 'orange', 'purple', 'yellow', 'pink', 'cyan', 'brown']
        colors = [base_colors[i % len(base_colors)] for i in range(len(model_names))]
