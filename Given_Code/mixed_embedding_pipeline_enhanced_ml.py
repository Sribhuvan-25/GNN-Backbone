...
class MixedEmbeddingPipeline:
    """
    Pipeline: GNN embeddings + nested-CV tuned ML models
    """
    ...
    def train_gnn_model(self, model_type, target_idx, data_list=None):
        """Train a single GNN model with K-fold CV"""
    ...
    def train_ml_models(self, embeddings, targets, target_idx):
        """
        Nested cross-validation for ML models
        """
        y = targets[:, target_idx] if targets.ndim==2 else targets
        print(f"\nTraining ML for: {self.target_names[target_idx]}")
        print(f"Emb: {embeddings.shape}, Y: {y.shape}")

        pipelines = {
            'LinearSVR': Pipeline([('scaler', StandardScaler()),
                                    ('regressor', LinearSVR(epsilon=0.1, tol=1e-4, C=1.0, max_iter=10000))]),
            'ExtraTrees': Pipeline([('regressor', ExtraTreesRegressor(n_estimators=100, random_state=42, n_jobs=-1))])
        }
        param_grids = {
            'LinearSVR': {
                'regressor__C': [0.5,1.0,2.0],
        ...
        for name, pipe in pipelines.items():
            print(f"\n{name} nested CV...")
            grid = GridSearchCV(pipe, param_grids[name], scoring='r2', cv=inner_cv, n_jobs=-1, verbose=1)
            fold_res = []
            for fold, (tr,te) in enumerate(outer_cv.split(embeddings),start=1):
                Xtr, Xte = embeddings[tr], embeddings[te]
                ytr, yte = y[tr], y[te]
                grid.fit(Xtr,ytr)
                best=grid.best_estimator_
                print(f"  Fold {fold}: best={grid.best_params_}")
                pred=best.predict(Xte)
                mse=mean_squared_error(yte,pred)
                rmse=np.sqrt(mse)
                r2v=r2_score(yte,pred)
                mae=mean_absolute_error(yte,pred)
                fold_res.append({'fold':fold,'mse':mse,'rmse':rmse,'r2':r2v,'mae':mae,'predictions':pred,'targets':yte})
                print(f"    Fold {fold}: R²={r2v:.3f}, RMSE={rmse:.3f}")
            all_pred=np.concatenate([f['predictions'] for f in fold_res])
            all_targ=np.concatenate([f['targets']     for f in fold_res])
            overall={'mse':mean_squared_error(all_targ,all_pred),
                     'rmse':np.sqrt(mean_squared_error(all_targ,all_pred)),
                     'r2':r2_score(all_targ,all_pred),
                     'mae':mean_absolute_error(all_targ,all_pred)}
            print(f"  {name} overall: R²={overall['r2']:.3f}, RMSE={overall['rmse']:.3f}\n")
            grid.fit(embeddings,y)
            final=grid.best_estimator_
            results[name]={'model':final,'fold_results':fold_res,'avg_metrics':overall}
            with open(f"{self.save_dir}/ml_models/{name}_{self.target_names[target_idx]}.pkl",'wb') as f:
                pickle.dump(final,f)
        return results

    def create_explainer_sparsified_graph(self, model, target_idx=0):
        print(f"\nCreating explainer graph for {self.target_names[target_idx]}")
        data_list = create_explainer_sparsified_graph(
            pipeline=self, model=model,
            target_idx=target_idx,
            importance_threshold=self.importance_threshold
        )
        print(f"Sparsified: {len(data_list)} samples")
        return data_list
    ...
    def run_pipeline(self):
        ...
        5. Train ML models on embeddings with nested CV
        6. Compare and analyze all results
        """
        print("\n" + "="*80)
        print("MIXED EMBEDDING PIPELINE - NESTED CV")
        print("="*80)
        ...
        # Print final summary
        print(f"\n{'='*80}")
        print("NESTED CV PIPELINE ANALYSIS COMPLETE!")
        print(f"{'='*80}")
        print("Results saved to: ./mixed_embedding_results_nestedcv/")
...
if __name__ == "__main__":
    pipeline = MixedEmbeddingPipeline(
        data_path="../Data/New_data.csv",
        k_neighbors=10,
        hidden_dim=64,
        num_epochs=200,
        num_folds=5,
        save_dir="./mixed_embedding_results_nestedcv",
        graph_mode='family',
        importance_threshold=0.2,
        use_fast_correlation=False,
